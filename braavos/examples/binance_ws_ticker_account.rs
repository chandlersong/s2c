...
use braavos::binance::bn_restful_commands::PostCommand;
...
async fn main() {
    ...
    // 
    let listen_key_command = CommandInfo::new_with_security(BinanceBase::PortfolioMargin,
                                                            BinancePath::PAPI(PmAPI::ListenKey),
                                                            &account.api_key,
                                                            &account.secret);

    let create_listen_key_command = PostCommand::<EmptyObject, ListenKeyResponse>::new();
    match create_listen_key_command.execute(listen_key_command, None, None).await {
        Ok(v) => {
