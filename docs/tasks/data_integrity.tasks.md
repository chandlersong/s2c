...
- [x] 任务3.3：实现 Supervisor 拉起 DataIntegrityChecker Actor
- [x] 任务3.4：实现 HealthState 状态管理与更新逻辑
- [x] 任务3.5：实现 GetHealthState Handler（只读查询接口）
- [x] 任务3.6：实现 Supervisor 接收修复结果消息，更新状态

### 任务4：Checker 组件
- [x] 任务4.1：定义 DataIntegrityChecker Actor 结构
- [x] 任务4.2：实现 Checker 启动时的首轮校验逻辑
- [x] 任务4.3：实现 interval 定时循环校验逻辑
- [x] 任务4.4：实现调用 ValidationStrategy 的校验流程
- [x] 任务4.5：实现校验超时控制与异常捕获
- [x] 任务4.6：实现产出 ValidationResult 事件投递给 RepairExecutor
